...
这一块现在是一个**会话面板**：每个开着的会话一行，同一 agent 的会话排在一起，宠物正在展示的那个排最前。每行可显示：agent 名、session id 后六位、会话名或项目名、模型（含 effort）、当前工具、上下文占用（荧光绿横条）、会话花费、5 小时 / 7 天额度用量、状态消息。每一项都可以在管理器的 Settings 里开关与排序；面板可设为常驻或只在有事发生时出现；宽度是宠物宽度的百分比（100–300%，滑杆 + 数值框 + 加减按钮），因此可以比宠物本体宽出三倍；状态消息的字号也可调（默认宠物下 8–20pt，它跟着宠物一起放大，而宽度只跟宠物、绝不跟字号）；行内文字可左/中/右对齐——**宠物本身不动**。状态消息的第二行仍是事件真正携带的信息：在等哪个工具、什么失败了，以及回合结束时**助手最后一条消息的预览**——Claude Code 通过 `last_assistant_message` 把它交给 hook，Codex 也是这么展示的。它按 Codex 的规则整理（折叠空白、截到 200 字符），只存在于内存，绝不写入任何日志文件；你输入过的内容永远不上屏。
