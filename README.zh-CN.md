...
公开的宠物契约定义了九条标准动画行加十六个注视姿态，每个都带**逐帧毫秒时长**。而且不均匀：`idle` 的创作值是 `280, 110, 110, 140, 140, 320`——那是一次呼吸，首末帧是中间帧的两三倍长——而 Codex 播放时会放慢 6 倍，本运行时与之相同。单一帧率表达不了，所以运行时存的是**逐帧时长**。

播放形状也照搬 Codex：某个状态的整行播**三遍**，然后交给 idle 行并循环下去。让宠物在整个任务期间不停重复工作姿势是抽搐，不是陪伴；它表达完就安静呼吸，正在发生什么由旁边的消息元素来说。系统要求 reduced motion 时，一切定格在单帧。
...
动画之外，宠物还带着 Codex 原版宠物才有的消息元素：精灵上方一小块状态，用同一套词汇——**Running**（第二行 "Thinking"）、等审批时的 **Needs input**（并写出是哪个工具）、回合结束的 **Ready**、出错的 **Blocked**。每条消息按 Codex 自己的时钟过期——干活 3 分钟、失败 1 小时、等待决定 24 小时、完成的回合 7 天——出现时窗口向上生长，宠物本体的位置纹丝不动。

第二行显示事件真正携带的信息：在等哪个工具、什么失败了，以及回合结束时**助手最后一条消息的预览**——Claude Code 通过 `last_assistant_message` 把它交给 hook，Codex 也是这么展示的。它按 Codex 的规则整理（折叠空白、截到 200 字符），只存在于内存，绝不写入任何日志文件；你输入过的内容永远不上屏。
